...
for split_index in args.splits:
    ...
    # SAVE MODEL
    if os.path.exist('check_point') == False:
        os.mkdir('check_point')
    if os.path.exist('check_point/{}'.format(data_set)) == False:
        os.mkdir('check_point/{}'.format(data_set))
    if os.path.exist('check_point/{}/LightGCN/split_{}'.format(data_set, split_index)) == False:
